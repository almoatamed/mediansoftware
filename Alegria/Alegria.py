...
class Toplevel1:
    ...
    query = b'H|\^&|||460213||||||||1|20100115155240'
                    ...
                    if data == b'\x05':
                        ...
                        try:
                            ...
                            if self.check_sum(self.frames[-1].split(b'\r')):
                                ...
                                print('Looper: checksum Failed')
                                self.show('\nchecksum False for ' + str(len(self.frames)))
                                self.frames.pop()
                                self.port.write(b'\x15')
                        except:
                            print('Looper: Error in checksum block')
                            self.show('\nchecksum False for ' + str(len(self.frames)))
                            self.frames.pop()
                            self.port.write(b'\x15')
                        print('looper: adding new frame')
                        self.frames.append(''.encode('ascii'))
                    elif data == b'\x04':
                        print('Looper: end of communication')
                        if not self.frames[-1]:
                            self.frames.pop()
                        for frame in self.frames:
                            print('Looper: Frame,',frame)
                        self.framer()
                        if 'R' in self.record_type:
                            print('Looper Storing Result')
